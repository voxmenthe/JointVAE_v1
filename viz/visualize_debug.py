...
class Visualizer():
    ...
    def reconstructions(self, data, size=(8, 8), filename='recon.png'):
        ...
        with torch.no_grad():
            ...
            input_data = data #input_data = Variable(data) # removed volatile=True and replaced as "with torch.no_grad()"
            print("viz reconstructions input data: ",type(input_data),input_data.shape)
            ...
            print("viz reconstructions recon_data shape: ", recon_data.shape)
            self.model.train()

            # Upper half of plot will contain data, bottom half will contain
            # reconstructions
            num_images = int(size[0] * size[1] / 2)
            print("viz reconstructions num_images: ", num_images)
            ...
            print("viz recon comparison shape: ",comparison.shape)
            print("viz recon comparison data shape: ",comparison.data.shape)
